...
const title = `<title>Zondicons</title>`;
...
const createHTMLTableBodyRows = (items: Icon[][]) => {
    ...
    items.forEach(row => {
        const columns = row.map(icon => `
            <td class="icon">
                <img src="../zondicons/${icon.name}" alt="${icon.name}">
            </td>
            <td class="iconName">${helper.toTitleCase(icon.name)}</td>
        `).join('');
        const tableRow = `
            <tr>
                ${columns}
...
const savePreview = (fileName: string, size: number, icons: Icon[][]) => {
    ...
    helper.createScreenshot(filePath, fileName).then(() => {
        console.log('> Zondicons:', helper.green(`Successfully created ${fileName} preview image!`));
    }).catch(() => {
        throw Error(helper.red(`Error while creating ${fileName} preview image`));
    });
};

const getIconDefinitionsMatrix = (icons: Icon[], size: number, excluded: string[] = []): Icon[][] => {
    const iconList = icons
                    .sort((a, b) => a.name.localeCompare(b.name))
                    .filter(i => excluded.indexOf(i.name) === -1);
 ...
 */
export const generatePreview = (name: string, icons: Icon[], size: number, excluded: string[] = ['zondicons.png']) => {
